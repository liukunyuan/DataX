...
//            return readerSplitConfigurations;
            ...
            List<Configuration> readerSplitConfigurations = new ArrayList<Configuration>();
            String[] indexArr = this.esIndex.split(",");
            int[] split = SplitUtil.dointSplit(0, indexArr.length - 1, adviceNumber);
            logger.info("切割的切片索引:{}", Arrays.toString(split));
            for (int i = 0; i < split.length-1; i++) {
                Configuration readerSplitConfiguration = this.readerSplitConfiguration.clone();
                readerSplitConfiguration.set("start",split[i]);
                readerSplitConfiguration.set("end",split[i+1]);
                readerSplitConfigurations.add(readerSplitConfiguration);
            }
...
//        private TransportClient client = null;
        ...
        public void init() {
            ...
            this.esPassword = readerSplitConfiguration.getString(Key.esPassword);
            this.query = readerSplitConfiguration.getString(Key.query);
            this.start = readerSplitConfiguration.getInt("start");
            this.end = readerSplitConfiguration.getInt("end");
        }
...
//            client.close();
        ...
        public void startRead(RecordSender recordSender) {
            String[] indexs = this.esIndex.split(",");
            for(int i=0;i< indexs.length;i++){
                if(start==0 && end==0) {
                    String indexName=indexs[i];
                    read(recordSender,indexName);
        ...
        public void read(RecordSender recordSender,String indexName) {
            EsReaderUtil iterator = esResultSet.search(indexName,esType,this.query);
            int totalSize = 0;
